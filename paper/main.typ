...
The Weakly Connected Components (WCC) algorithm, which identifies groups of nodes that are connected to each other through any path regardless of direction, failed to produce meaningful results. With an average path length of 1.58 and a maximum of only 3.0, nearly all nodes were placed into a single giant component, providing no useful partitioning of the network. When ran, every single node was assigned to the same community.

Similarly, the Louvain Modularity algorithm, which detects communities by optimizing modularity (the density of connections within communities versus connections between communities), could not identify distinct communities. The algorithm relies on finding areas with higher internal than external connectivity, but with most instances being only one or two hops away from any other instance, there were no clear boundaries for community formation. When we ran this algorithm, it created two communities, with their only deliniation being the alphabetical order of their domain.

The Label Propagation algorithm, which works by propagating labels through the network and forming communities of nodes that share the same label, also failed to converge to meaningful communities. The extremely high interconnectivity meant that labels propagated too quickly across the entire network, again resulting in a single community.
